##running the main pipeline for the ecolevel regions 
...
defol_summary <- centroids_hCID_M16_kCIDs_Ecolevel3 %>%
...
finish <- 2019
  ...
  cluster_ids<-unique(defol_summary$NA_L3CODE)
  nclusters<-length(cluster_ids)
  all <- tibble(NA_L3CODE=sort(rep((cluster_ids),finish-start+1)),
                SURVEY_YR=rep((start:finish),nclusters))
  
  defol_summary_complete<-full_join(defol_summary,all,by=c("SURVEY_YR","NA_L3CODE")) %>%
...
defol_summary_complete_outbrkyrs <- tibble()
  ...
  for (i in 1:length(unique(defol_summary_complete$NA_L3CODE))) {  ##this loop assigns each cell in defol_summary_complete a yr_of_outbreak value
    
    filtered <- defol_summary_complete %>%
      filter(NA_L3CODE == unique(defol_summary_complete$NA_L3CODE)[i]) %>%
...
write_csv(defol_summary_complete_outbreakIDs, file = "./Cluster analysis/dissolved/clusters_dispersal.gdb/defol_summaries/defol_summary_ecolevel_post1988.csv")
defol_summary_complete_outbreakIDs_level3eco <- read_csv("./Cluster analysis/dissolved/clusters_dispersal.gdb/defol_summaries/defol_summary_ecolevel_post1988.csv") %>%
  mutate(eco_level = 3)
colnames(defol_summary_complete_outbreakIDs_level3eco)[1]<-"Ecoregion_Code"
