##running the main pipeline for the level 1 ecoregions
...
centroids_M8_Ecolevel1 <- read_csv("./Cluster analysis/dissolved/clusters_dispersal.gdb/centroids_NA_L1-2CODE.csv") %>%
  filter(SURVEY_YR > 1989 & NA_L1CODE > -1)

####code from previous pipeline
##calculating "final defol data" for the entire polygons set at once
defol_summary <- centroids_M8_Ecolevel1 %>%
  group_by(NA_L1CODE, SURVEY_YR) %>%
...
cluster_ids<-unique(defol_summary$NA_L1CODE)
nclusters<-length(cluster_ids)
all <- tibble(NA_L1CODE=sort(rep((cluster_ids),finish-start+1)),
              SURVEY_YR=rep((start:finish),nclusters))

defol_summary_complete<-full_join(defol_summary,all,by=c("SURVEY_YR","NA_L1CODE")) %>%
...
for (i in 1:length(unique(defol_summary_complete$NA_L1CODE))) {  ##this loop assigns each cell in defol_summary_complete a yr_of_outbreak value
  
  filtered <- defol_summary_complete %>%
    filter(NA_L1CODE == unique(defol_summary_complete$NA_L1CODE)[i]) %>%
    mutate(yr_of_outbreak = 0)
  
  for (j in 1:length(filtered$NA_L1CODE)) {
    if (j==1) {
...
filtered <- filtered %>%
  select(NA_L1CODE, SURVEY_YR, yr_of_outbreak, outbreak_ID)

defol_summary_complete_outbreakIDs <- defol_summary_complete_outbrkyrs %>%
  left_join(filtered, by = c("NA_L1CODE", "yr_of_outbreak", "SURVEY_YR"))
...
write_csv(defol_summary_complete_outbreakIDs, file = "./Cluster analysis/dissolved/clusters_dispersal.gdb/defol_summaries/defol_summary_level1eco_post1988.csv")
defol_summary_complete_outbreakIDs_level1eco <- read_csv("./Cluster analysis/dissolved/clusters_dispersal.gdb/defol_summaries/defol_summary_level1eco_post1988.csv")%>%
  mutate(eco_level = 1)
colnames(defol_summary_complete_outbreakIDs_level1eco)[1]<-"Ecoregion_Code"
defol_summary_complete_outbreakIDs_level1eco$Ecoregion_Code <- as.character(defol_summary_complete_outbreakIDs_level1eco$Ecoregion_Code)
